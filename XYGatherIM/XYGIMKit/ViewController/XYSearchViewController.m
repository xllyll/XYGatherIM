...
static XYSearchViewController *_instance;
...
- (void)commonInit {
    ...
    self.searchBar = [XYSearchBar defaultSearchBar];
...
+ (instancetype)sharedInstance {
    if (!_instance) {
        _instance = [[XYSearchViewController alloc] initWithNibName:nil bundle:nil];
    }
...
- (void)showInViewController:(UIViewController *)controller fromSearchBar:(UISearchBar *)_fromSearchBar {
    ...
    fromSearchBarFrame = [fromSearchBar convertRect:fromSearchBar.bounds toView:[XYUtils currentWindow]];
    self.toNavigationBarView.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, CGRectGetMaxY(fromSearchBarFrame));
