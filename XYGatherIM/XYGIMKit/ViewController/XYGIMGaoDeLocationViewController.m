...
#import "XYLocationSearchResultController.h"
#import "XYUtils.h"
...
@interface XYGIMGaoDeLocationViewController ()<MAMapViewDelegate, AMapSearchDelegate,AMapLocationManagerDelegate, XYSearchControllerDelegate, UISearchBarDelegate, CLLocationManagerDelegate, UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic) XYGIMAroundSearchTableStyle reGeocodeStyle;
@property (nonatomic) XYGIMAroundSearchTableStyle POISearchStyle;
...
@implementation XYGIMGaoDeLocationViewController{
    ...
    XYLocationSearchResultController *resultController;
...
- (void)viewDidLoad {
    ...
    _searchBar = [XYSearchBar defaultSearchBar];
    ...
    CALayer *line = [XYUtils lineWithLength:SCREEN_WIDTH atPoint:CGPointZero];
    ...
    line = [XYUtils lineWithLength:SCREEN_WIDTH atPoint:CGPointZero];
    line.backgroundColor = _tableView.separatorColor.CGColor;
...
- (void)send:(UIBarButtonItem *)item {
    ...
    __strong id<XYGIMLocationViewDelegate> delegate = self.delegate;
    
    XYNMessage *messageModel = [self sendLocationWithSnapshot:nil];
    
    [[XYLocationManager sharedManager] takeCenterSnapshotFromMapView:self.mapView withCompletionBlock:^(UIImage *resultImage, CGRect rect) {
...
- (XYNMessage *)sendLocationWithSnapshot:(UIImage *)snapshot {
    NSString *address;
    NSString *name;
    ...
    if (!_curCenterReGeocode) {
        ...
        if (distance > 100) {
            ...
            [[XYLocationManager sharedManager] getLocationNameAndAddressFromReGeocode:_curCenterReGeocode name:&name address:&address];
        }
    }
...
- (void)netConnectionStatusChanged:(NSNotification *)notification {
    BOOL isConnected = [notification.userInfo[@"connectionState"] integerValue] == XYGIMConnectionStateConnected;
...
- (void)promptNoAuthorizationAlert {
    WEAK_SELF;
    [XYUtils showMessageAlertWithTitle:nil message:LOCATION_AUTHORIZATION_DENIED_TEXT actionTitle:@"确定" actionHandler:^{
        [weakSelf cancel:nil];
    }];
...
- (void)onPOISearchDone:(AMapPOISearchBaseRequest *)request response:(AMapPOISearchResponse *)response {
    ...
    if (response.pois.count < request.offset) {
        ...
        [self setSearchTableStyle:kXYGIMAroundSearchTableStylePOIAllPageSearchComplete];
    }else {
        [self.allMapPOIs addObjectsFromArray:response.pois];
        [self setSearchTableStyle:kXYGIMAroundSearchTableStylePOIPageSearchComplete];
    }
...
- (void)onReGeocodeSearchDone:(AMapReGeocodeSearchRequest *)request response:(AMapReGeocodeSearchResponse *)response {
    ...
    self.searchTableStyle = kXYGIMAroundSearchTableStyleReGeocodeComplete;
}
...
- (void)fetchPOIAroundCenterCoordinate {
    ...
    self.searchTableStyle = kXYGIMAroundSearchTableStyleBeginSearch;
...
- (void)setSearchTableStyle:(XYGIMAroundSearchTableStyle)searchTableStyle {
    switch (searchTableStyle) {
        case kXYGIMAroundSearchTableStyleBeginSearch: {
        ...
        case kXYGIMAroundSearchTableStyleReGeocodeComplete: {
            _reGeocodeStyle = searchTableStyle;
            ...
            if (_POISearchStyle == kXYGIMAroundSearchTableStyleBeginSearch) {
        ...
        case kXYGIMAroundSearchTableStylePOIPageSearchComplete:{
            _POISearchStyle = searchTableStyle;
        ...
        case kXYGIMAroundSearchTableStylePOIAllPageSearchComplete: {
            _POISearchStyle = searchTableStyle;
            if (self.tableView.tableFooterView == footerView) {
                self.tableView.tableFooterView = blankView;
                [footerIndicator stopAnimating];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    //开始加载新的数据
    if ((_POISearchStyle == kXYGIMAroundSearchTableStylePOIPageSearchComplete) && (!footerIndicator.isAnimating) && (footerIndicator.hidden) && (_tableView.tableFooterView == footerView) && (scrollView.contentOffset.y + scrollView.frame.size.height + 2 >= scrollView.contentSize.height)) {
        footerIndicator.hidden = NO;
        [footerIndicator startAnimating];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        if (indexPath.row == 0) {
            ...
            cell = [[XYLocationTableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:ID2];
        }
    }
    ...
    if (isHeadRow) {
        ...
        ((XYLocationTableViewCell *)cell).poiModel = model;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        XYLocationTableViewCell *locationCell = (XYLocationTableViewCell *)cell;
...
- (BOOL)searchBarShouldBeginEditing:(UISearchBar *)searchBar {
    XYSearchViewController *vc = [XYSearchViewController sharedInstance];
    vc.delegate = self;
    if (!resultController) {
        resultController = [[XYLocationSearchResultController alloc] init];
...
- (void)didRowWithModelSelected:(AMapPOI *)poiModel {
    ...
    [[XYSearchViewController sharedInstance] dismissSearchController];
}
