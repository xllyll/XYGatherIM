//
//  XYGIMChatAssetDisplayController.m
//  XYGatherIM
//
...
#import "XYGIMVideoPlaybackController.h"
#import "XYGIMImageBottomBar.h"
#import "XYGIMImageAnimationView.h"
#import "XYGIMActionSheet.h"
#import "XYUtils.h"
#import "XYColors.h"
#import "XYConfig.h"
#import "XYGIMChatImageScrollView.h"
#import "XYGIMVideoDisplayView.h"

#define XYGIMChatManagerMessageModelKey @"LLChatManagerMessageModelKey"
//每张图片之间的间隔
#define INTERVAL 25
...
typedef UIView<XYGIMAssetDisplayView> XYGIMAssetView;
typedef NS_ENUM(NSInteger, XYGIMAssetBottomBarStyle) {
    kXYGIMAssetBottomBarStyleNone = 0,
    kXYGIMAssetBottomBarStyleImageHide,
    kXYGIMAssetBottomBarStyleImageShow,
    kXYGIMAssetBottomBarStyleVideo
};
@interface XYGIMChatAssetDisplayController ()<UIScrollViewDelegate, XYGIMVideoPlaybackDelegate>
@property (nonatomic) XYGIMAssetView *curAssetView;
@property (nonatomic) UIScrollView *scrollView;

@property (nonatomic) UIButton *button;
@property (nonatomic) UIButton *closeButton;

@property (nonatomic) XYGIMImageBottomBar *imageBottomBar;
@property (nonatomic) UIView *videoBottomBar;
//@property (nonatomic) UIView *statusView;

@property (nonatomic) XYGIMImageAnimationView *imageAnimationView;

@property (nonatomic) NSMutableArray<XYGIMAssetView *> *allAssetViews;

@property (nonatomic) XYGIMVideoPlaybackController *videoPlaybackController;
@end

@implementation XYGIMChatAssetDisplayController{
    NSInteger length;
    NSInteger scroll_width;
    CGPoint screenCenter;
    BOOL scrollToTop;
    __weak XYGIMActionSheet *actionSheet;
    
    NSTimer *timer;
    XYGIMAssetBottomBarStyle bottomBarStyle;
...
- (void)viewDidLoad {
    ...
    [XYUtils configAudioSessionForPlayback];
    ...
    }else if ((_curShowMessageModel.bodyType == XYGIMMessageBodyTypeVideo) &&
              ((_curShowMessageModel.messageDownloadStatus == XYGIMDownloadStatusPending) || (_curShowMessageModel.messageDownloadStatus ==  XYGIMDownloadStatusFailed))){
...
- (void)setupViews {
    ...
    for (int i = 0; i < length; i++) {
        XYGIMChatImageScrollView *imageScrollView = [[XYGIMChatImageScrollView alloc] initWithFrame:SCREEN_FRAME];
        imageScrollView.hidden = YES;
        imageScrollView.delegate = self;
        [_scrollView addSubview:imageScrollView];
        [_allAssetViews addObject:imageScrollView];
        
        XYGIMVideoDisplayView *videoView = [[XYGIMVideoDisplayView alloc] initWithFrame:SCREEN_FRAME];
...
- (void)showAllAssets {
    ...
    for (NSInteger i = 0; i< length; i++) {
        XYGIMAssetView *assetView = [self showAssetViewWithAssetIndex:assetIndex];
        assetIndex++;
        if (i == showIndex - from)
    ...
    bottomBarStyle = kXYGIMAssetBottomBarStyleNone;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([_curAssetView isKindOfClass:[XYGIMChatImageScrollView class]]) {
        XYGIMChatImageScrollView *imageScrollView = (XYGIMChatImageScrollView *)_curAssetView;
        
        CGRect frame = imageScrollView.imageView.frame;
        CGRect fromRect = [[XYUtils currentWindow] convertRect:self.originalWindowFrame toView:_curAssetView];
        imageScrollView.imageView.frame = fromRect;
        
        [XYUtils currentWindow].userInteractionEnabled = NO;
        [UIView animateWithDuration:DEFAULT_DURATION
                         ...
                         completion:^(BOOL finished) {
                             ...
                             [XYUtils currentWindow].userInteractionEnabled = YES;
                         }];
        
    }else if ([_curAssetView isKindOfClass:[XYGIMVideoDisplayView class]]) {
        XYGIMVideoDisplayView *videoDisplayView = (XYGIMVideoDisplayView *)_curAssetView;
        CGRect frame = videoDisplayView.imageView.frame;
        CGRect fromRect = [[XYUtils currentWindow] convertRect:self.originalWindowFrame toView:self.view];
...
- (void)layoutAssetViews:(CGSize)size {
    ...
    for (UIView<XYGIMAssetDisplayView> *view in self.scrollView.subviews) {
        if (view.assetIndex >= 0) {
            view.frame = CGRectMake(view.assetIndex * scroll_width, 0, size.width, size.height);
            if (view.messageBodyType == XYGIMMessageBodyTypeImage) {
                XYGIMChatImageScrollView *imageScrollView = (XYGIMChatImageScrollView *)view;
                ...
                [(XYGIMChatImageScrollView *)view layoutImageView:size];
            }
        }
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    UIView<XYGIMAssetDisplayView> *assetView;
    for (UIView<XYGIMAssetDisplayView> *view in self.scrollView.subviews) {
...
- (void)changeAssetViewToInitialState:(UIView<XYGIMAssetDisplayView> *)view {
    if (view.messageBodyType == XYGIMMessageBodyTypeImage) {
        XYGIMChatImageScrollView *curImageScrollView = (XYGIMChatImageScrollView *)self.curAssetView;
    ...
    }else if (view.messageBodyType == XYGIMMessageBodyTypeVideo) {
        XYGIMVideoDisplayView *videoDisplayView = (XYGIMVideoDisplayView *)view;
        videoDisplayView.videoPlaybackStatus = kXYGIMVideoPlaybackStatusPicture;
...
- (XYGIMAssetView *)assetViewWithAssetIndex:(NSInteger)assetIndex {
    for (UIView<XYGIMAssetDisplayView> *view in _allAssetViews) {
...
- (XYGIMAssetView *)assetViewWithMessageModel:(XYNMessage *)messageModel {
    for (UIView<XYGIMAssetDisplayView> *view in _allAssetViews) {
...
- (XYGIMAssetView *)hideAssetViewWithAssetIndex:(NSInteger)assetIndex {
    for (XYGIMAssetView *view in _allAssetViews) {
...
- (XYGIMAssetView *)showAssetViewWithAssetIndex:(NSInteger)assetIndex {
    
    for (XYGIMAssetView *view in _allAssetViews) {
        if (view.assetIndex == assetIndex) {
            if (view.messageBodyType == XYGIMMessageBodyTypeVideo) {
                ((XYGIMVideoDisplayView *)view).needAnimation = YES;
                [self checkVideoDownloadStatus:(XYGIMVideoDisplayView *)view];
            }
            view.hidden = NO;
            return view;
        }
    ...
    XYNMessage *model = _allAssets[assetIndex];
    XYGIMAssetView *assetView;
    for (XYGIMAssetView *view in _allAssetViews) {
        if (view.hidden && view.messageBodyType == model.bodyType) {
            assetView = view;
            assetView.hidden = NO;
            break;
        }
    ...
    if (model.bodyType == XYGIMMessageBodyTypeImage) {
        XYGIMChatImageScrollView *imageScrollView = (XYGIMChatImageScrollView *)assetView;
    ...
    }else if (model.bodyType == XYGIMMessageBodyTypeVideo) {
        XYGIMVideoDisplayView *videoDisplayView = (XYGIMVideoDisplayView *)assetView;
...
- (UIView *)viewForZoomingInScrollView:(UIScrollView *)scrollView {
    if (![scrollView isKindOfClass:[XYGIMChatImageScrollView class]])
        return nil;
    return ((XYGIMChatImageScrollView *)scrollView).imageView;
}
...
- (void)handleZoom:(UITapGestureRecognizer *)tap {
    ...
    XYGIMChatImageScrollView *scrollView = (XYGIMChatImageScrollView *)_curAssetView;
...
- (void)scrollViewDidZoom:(XYGIMChatImageScrollView *)scrollView {
...
- (UIView *)imageBottomBar {
    if (!_imageBottomBar) {
        _imageBottomBar = [[NSBundle mainBundle] loadNibNamed:@"XYGIMImageBottomBar" owner:self options:nil][0];
...
- (UIView *)videoBottomBar {
    if (!_videoBottomBar) {
        _videoPlaybackController = [[XYGIMVideoPlaybackController alloc] initWithNibName:@"XYGIMVideoPlaybackController" bundle:nil];
...
- (XYGIMImageAnimationView *)imageAnimationView {
    if (!_imageAnimationView) {
        _imageAnimationView = [[XYGIMImageAnimationView alloc] initWithFrame:CGRectMake(0, 0, 35, 35)];
...
- (void)showBottomBar {
    ...
    if ([_curAssetView isKindOfClass:[XYGIMChatImageScrollView class]]) {
        ...
        if (_curShowMessageModel.isVideoPlayable) {
            ...
            if (bottomBarStyle == kXYGIMAssetBottomBarStyleImageHide) {
                [_videoPlaybackController setBackgroundViewVisible:NO];
                [_videoPlaybackController showControlView:NO];
            }else if (bottomBarStyle == kXYGIMAssetBottomBarStyleImageShow) {
                [_videoPlaybackController setBackgroundViewVisible:NO];
...
- (void)hideImageBottomBar:(NSTimer *)timer {
    bottomBarStyle = kXYGIMAssetBottomBarStyleImageHide;
    
    [_imageBottomBar setBottomBarStyle:kXYGIMImageBottomBarStyleHide animated:YES];
}
...
- (void)checkDownloadStatus {
    ...
    if (_curAssetView.messageBodyType == XYGIMMessageBodyTypeImage) {
        switch (_curShowMessageModel.messageDownloadStatus) {
            case XYGIMDownloadStatusWaiting:
            case XYGIMDownloadStatusDownloading: {
                //用户点击了查看原图
                if ([downloadingMessageIds containsObject:_curShowMessageModel.messageId]) {
                    bottomBarStyle = kXYGIMAssetBottomBarStyleImageHide;
                    [self.imageBottomBar setBottomBarStyle:kXYGIMImageBottomBarStyleDownloading animated:YES];
                ...
                break;
            case XYGIMDownloadStatusFailed:
                if ([downloadFailedMessageIds containsObject:_curShowMessageModel.messageId]) {
                    
                    bottomBarStyle = kXYGIMAssetBottomBarStyleImageHide;
                    [self.imageBottomBar setBottomBarStyle:kXYGIMImageBottomBarStyleHide animated:YES];
                    
                }else {
                    bottomBarStyle = kXYGIMAssetBottomBarStyleImageShow;
                    [self.imageBottomBar setBottomBarStyle:kXYGIMImageBottomBarStyleDownloadFullImage animated:YES];
                    [self.imageBottomBar setDownloadFullImageSize:[XYUtils sizeStringWithStyle:nil size:[_curShowMessageModel fileAttachmentSize]]];
                }
                break;
            case XYGIMDownloadStatusPending:
                //TODO:FIX
//                [[LLChatManager sharedManager] asynDownloadMessageAttachments:_curShowMessageModel progress:nil completion:nil];
//                needShowAnimationView = YES;
                break;
            case XYGIMDownloadStatusSuccessed: {
                bottomBarStyle = kXYGIMAssetBottomBarStyleImageShow;
                [self.imageBottomBar setBottomBarStyle:kXYGIMImageBottomBarStyleMore animated:YES];
                [timer invalidate];
    ...
    }else if (_curAssetView.messageBodyType == XYGIMMessageBodyTypeVideo) {
        needShowAnimationView = NO;
        bottomBarStyle = kXYGIMAssetBottomBarStyleVideo;
        ...
        if (_curShowMessageModel.isVideoPlayable) {
            ...
            switch (_curShowMessageModel.messageDownloadStatus) {
                case XYGIMDownloadStatusWaiting:
                    [videoDisplayView setVideoDownloadStyle:kXYGIMVideoDownloadStyleWaiting];
                    break;
                case XYGIMDownloadStatusDownloading:
                    [videoDisplayView setVideoDownloadStyle:kXYGIMVideoDownloadStyleDownloading];
...
- (void)checkVideoDownloadStatus:(XYGIMVideoDisplayView *)videoDisplayView {
    switch (videoDisplayView.messageModel.messageDownloadStatus) {
        case XYGIMDownloadStatusPending:
            [videoDisplayView setVideoDownloadStyle:kXYGIMVideoDownloadStylePending];
            break;
        case XYGIMDownloadStatusSuccessed:
            [videoDisplayView setVideoDownloadStyle:kXYGIMVideoDownloadStyleDownloadSuccess];
            break;
        case XYGIMDownloadStatusFailed: {
            if ([downloadFailedMessageIds containsObject:videoDisplayView.messageModel.messageId]) {
                [videoDisplayView setVideoDownloadStyle:kXYGIMVideoDownloadStyleFailed];
            }else {
                [videoDisplayView setVideoDownloadStyle:kXYGIMVideoDownloadStylePending];
            }
        }
...
- (void)playerScrubberWillChange:(NSURL *)videoURL {
    if ([_curShowMessageModel.fileLocalPath isEqualToString:videoURL.path]) {
        XYGIMVideoDisplayView *videoDisplayView = (XYGIMVideoDisplayView *)_curAssetView;
        videoDisplayView.videoPlaybackStatus = kXYGIMVideoPlaybackStatusVideo;
    }
}
    ...
    if (rate != 0.f) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if ([weakSelf.curShowMessageModel.fileLocalPath isEqualToString:videoURL.path]) {
                XYGIMVideoDisplayView *videoDisplayView = (XYGIMVideoDisplayView *)weakSelf.curAssetView;
                videoDisplayView.videoPlaybackStatus = kXYGIMVideoPlaybackStatusVideo;
            }
        });
    }
...
- (void)registerChatManagerNotification {
    //[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(messageDownloadHandler:) name:XYGIMMessageDownloadStatusChangedNotification object:nil];
}

- (void)downloadAttachmentForMessageModel:(XYNMessage *)messageModel {
...
- (void)imageDownloadNotification:(XYNMessage *)messageModel {
    WEAK_SELF;
    dispatch_async(dispatch_get_main_queue(), ^{
        STRONG_SELF;
        if (messageModel.messageDownloadStatus == XYGIMDownloadStatusSuccessed) {
            [strongSelf->downloadingMessageIds removeObject:messageModel.messageId];
            XYGIMAssetView *imageView = [weakSelf assetViewWithMessageModel:messageModel];
            [imageView setMessageModel:messageModel];
        }else if (messageModel.messageDownloadStatus == XYGIMDownloadStatusFailed) {
            [strongSelf->downloadingMessageIds removeObject:messageModel.messageId];
            [strongSelf->downloadFailedMessageIds addObject:messageModel.messageId];
            XYGIMAssetView *imageView = [weakSelf assetViewWithMessageModel:messageModel];
            [(XYGIMChatImageScrollView *)imageView setDownloadFailImage];
        }
...
- (void)videoDownloadNotification:(XYNMessage *)messageModel {
    WEAK_SELF;
    dispatch_async(dispatch_get_main_queue(), ^{
        STRONG_SELF;
        XYGIMVideoDisplayView *videoDisplayView = (XYGIMVideoDisplayView *)[weakSelf assetViewWithMessageModel:messageModel];
...
- (void)messageDownloadHandler:(NSNotification *)notification {
    XYNMessage *messageModel = notification.userInfo[XYGIMChatManagerMessageModelKey];
    if (!messageModel)
        return;
    
    if (messageModel.bodyType == XYGIMMessageBodyTypeVideo) {
        [self videoDownloadNotification:messageModel];
    }else if (messageModel.bodyType == XYGIMMessageBodyTypeImage) {
...
- (IBAction)imagedownload:(UIButton *)button {
    if ([button.titleLabel.text rangeOfString:@"查看原图"].location != NSNotFound) {
        ...
        bottomBarStyle = kXYGIMAssetBottomBarStyleImageHide;
        [_imageBottomBar setBottomBarStyle:kXYGIMImageBottomBarStyleDownloading animated:YES];
...
- (void)HUDDidTapped:(XYGIMVideoDownloadStatusHUD *)HUD {
    if ((_curShowMessageModel.bodyType == XYGIMMessageBodyTypeVideo) &&
        ((_curShowMessageModel.messageDownloadStatus == XYGIMDownloadStatusPending) || (_curShowMessageModel.messageDownloadStatus == XYGIMDownloadStatusFailed))) {
        [self videodownload];
    }
}
...
- (void)handleAction:(UILongPressGestureRecognizer *)gesture {
    if (gesture.state == UIGestureRecognizerStateBegan) {
        if (_curShowMessageModel.bodyType == XYGIMMessageBodyTypeImage) {
...
- (void)showImageActionSheet:(XYNMessage *)model {
    XYGIMActionSheet *aActionSheet = [[XYGIMActionSheet alloc] initWithTitle:nil];
    actionSheet = aActionSheet;
    
    XYGIMActionSheetAction *action1 = [XYGIMActionSheetAction
                                    actionWithTitle:@"发送给朋友"
                                    handler:^(XYGIMActionSheetAction *action) {
                                        
                                    }];
    
    XYGIMActionSheetAction *action2 = [XYGIMActionSheetAction
                                    actionWithTitle:@"收藏"
                                    handler:^(XYGIMActionSheetAction *action) {
                                        
                                    }];
    
    XYGIMActionSheetAction *action3 = [XYGIMActionSheetAction
                                    actionWithTitle:@"保存图片"
                                    handler:^(XYGIMActionSheetAction *action) {
                                        [XYUtils saveImageToPhotoAlbum:model.fullImage];
                                    }] ;
    
    XYGIMActionSheetAction *action4 = [XYGIMActionSheetAction
...
- (void)showVideoActionSheet:(XYNMessage *)model {
    XYGIMActionSheet *aActionSheet = [[XYGIMActionSheet alloc] initWithTitle:nil];
    actionSheet = aActionSheet;
    ...
    XYGIMActionSheetAction *action3 = [XYGIMActionSheetAction
                                    ...
                                    handler:^(XYGIMActionSheetAction *action) {
                                        [XYUtils saveVideoToPhotoAlbum:model.fileLocalPath];
                                    }] ;
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    _imageBottomBar.hidden = YES;
    _videoBottomBar.hidden = YES;
    //    if (_curAssetView.messageBodyType == kXYGIMMessageBodyTypeVideo) {
    //        XYGIMVideoDisplayView *videoDisplayView = (XYGIMVideoDisplayView *)_curAssetView;
