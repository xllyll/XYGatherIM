...
@interface _LLInternal_Data_XYLocationManager : NSObject
...
@implementation _LLInternal_Data_XYLocationManager


@end

@interface XYLocationManager ()<AMapSearchDelegate>

@property (nonatomic) MAMapView *mapView;

@property (nonatomic) AMapSearchAPI *reGeocodeSearch;

@property (nonatomic) NSMutableArray<_LLInternal_Data_XYLocationManager *> *allRequests;


@end
@implementation XYLocationManager
+ (instancetype)sharedManager {
    static XYLocationManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[XYLocationManager alloc] init];
    });
    return manager;
}
...
- (void)AMapSearchRequest:(AMapReGeocodeSearchRequest *)request didFailWithError:(NSError *)error {
    ...
    @synchronized (self) {
        for (NSInteger i = 0, r = _allRequests.count; i < r; i++) {
            _LLInternal_Data_XYLocationManager *data = _allRequests[i];
...
- (void)reGeocodeFromCoordinate:(CLLocationCoordinate2D)coordinate2D completeCallback:(void (^)(AMapReGeocode *address, CLLocationCoordinate2D coordinate2D))completeCallback {
    ...
    _LLInternal_Data_XYLocationManager *data = [_LLInternal_Data_XYLocationManager new];
...
-(void)navigationUsingGaodeMapFromLocation:(CLLocationCoordinate2D)fromCoordinate2D toLocation:(CLLocationCoordinate2D)toCoordinate2D destinationName:(NSString *)destinationName {
    NSString *urlString = [[NSString stringWithFormat:@"iosamap://path?sourceApplication=%@&sid=BGVIS1&slat=%f&slon=%f&did=BGVIS2&dlat=%f&dlon=%f&dname=%@&dev=0&m=0&t=0",[XYUtils appName], fromCoordinate2D.latitude, fromCoordinate2D.longitude, toCoordinate2D.latitude, toCoordinate2D.longitude, destinationName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
