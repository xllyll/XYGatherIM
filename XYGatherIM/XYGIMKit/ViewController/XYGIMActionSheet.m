...
typedef NS_ENUM(NSInteger, XYGIMActionItemType) {
    kXYGIMActionItemTypeTitle,
    kXYGIMActionItemTypeAction,
    kXYGIMActionItemTypeCancel
};
...
XYGIMActionSheetAction *LL_ActionSheetSeperator = nil;

@implementation XYGIMActionSheetAction

+ (void)load {
    if (!LL_ActionSheetSeperator)
        LL_ActionSheetSeperator = [[XYGIMActionSheetAction alloc] init];
}

+ (instancetype)actionWithTitle:(NSString *)title handler:(void (^)(XYGIMActionSheetAction *action))handler {
    return [self actionWithTitle:title handler:handler style:kXYGIMActionStyleDefault];
}

+ (instancetype)actionWithTitle:(NSString *)title handler:(ACTION_BLOCK)handler style:(XYGIMActionStyle)style {
    
    XYGIMActionSheetAction * action = [[XYGIMActionSheetAction alloc] init];
    action.title = title;
...
@interface XYGIMActionSheet ()

@property (nonatomic, copy) NSString *title;

@property (nonatomic) UIView *contentView;

@property (nonatomic) NSMutableArray<XYGIMActionSheetAction *> *actions;

@end


@implementation XYGIMActionSheet {
    CGRect _windowBounds;
}

- (instancetype)initWithTitle:(NSString *)title {
    self = [super initWithFrame:[XYUtils screenFrame]];
    if (self) {
        self.title = title;
...
- (void)showInWindow:(UIWindow *)window {
    ...
    if (!actionSheetContainer.superview){
        if (!window || window == [XYUtils popOverWindow]) {
            [XYUtils addViewToPopOverWindow:actionSheetContainer];
        }else {
            [window addSubview:actionSheetContainer];
        }
    }
    
    _windowBounds = [XYUtils screenFrame];
...
- (void)setupViews {
    ...
    if (self.title && self.title.length > 0) {
        UIView *titleView = [self createItemWithType:kXYGIMActionItemTypeTitle
                                                data:nil];
        [self.contentView addSubview:titleView];
        _y = CGRectGetMaxY(titleView.frame);
    }
    
    for (XYGIMActionSheetAction *action in self.actions) {
        if (action == LL_ActionSheetSeperator) {
            _y += CANCEL_BAR_GAP;
        }else {
            UIView *actionButton = [self createItemWithType:kXYGIMActionItemTypeAction
                                                       data:action];
    ...
    UIView *cancelButton = [self createItemWithType:kXYGIMActionItemTypeCancel
                                               data:nil];
...
- (CGFloat)barHeightForType:(XYGIMActionItemType)type {
    switch(type) {
        case kXYGIMActionItemTypeTitle:
            return TITLE_BAR_HEIGHT;
        case kXYGIMActionItemTypeAction:
            return ACTION_BAR_HEIGHT;
        case kXYGIMActionItemTypeCancel:
            return CANCEL_BAR_HEIGHT;
    }
}


- (UIView *)createItemWithType:(XYGIMActionItemType)type data:(XYGIMActionSheetAction *)data {
    CGRect frame = CGRectMake(0, 0, CGRectGetWidth(_windowBounds), [self barHeightForType:type]);
    
    if (type == kXYGIMActionItemTypeTitle) {
        ...
        if (type == kXYGIMActionItemTypeAction) {
            ...
            UIColor *buttonTitleColor = data.style == kXYGIMActionStyleDefault ? ACTION_FONT_DEFAULT_COLOR : ACTION_FONT_DESTRUCTIVE_COLOR;
            ...
            line.frame = CGRectMake(0, 0, SCREEN_WIDTH, 1/[XYUtils screenScale]);
            [button.layer addSublayer:line];
        }else if (type == kXYGIMActionItemTypeCancel) {
...
- (void)tapAction:(UIButton *)sender {
    XYGIMActionSheetAction *action = self.actions[sender.tag];
    if (action.handler) {
...
- (void)close {
    [UIView animateWithDuration:0.25 animations:^{
        ...
        if (actionSheetContainer.subviews.count == 0){
            if (actionSheetContainer.window == [XYUtils popOverWindow])
                [XYUtils removeViewFromPopOverWindow:actionSheetContainer];
            else {
