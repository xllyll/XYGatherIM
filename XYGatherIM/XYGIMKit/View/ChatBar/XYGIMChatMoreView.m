...
@interface XYGIMChatMoreView ()<UIScrollViewDelegate>
...
@implementation XYGIMChatMoreView
...
- (void)itemClickAction:(XYGIMChatMoreItem *)item{
...
- (void)setupItems{
    ...
    [self.titles enumerateObjectsUsingBlock:^(NSString *obj, NSUInteger idx, BOOL *stop) {
        ...
        XYGIMChatMoreItem *item = [[XYGIMChatMoreItem alloc] initWithFrame:CGRectMake(startX, startY, self.itemSize.width, self.itemSize.height)];
...
- (void)setDataSource:(id<XYGIMChatMoreViewDataSource>)dataSource{
...
- (UIScrollView *)scrollView{
    ...
    _scrollView.frame = CGRectMake(0, self.edgeInsets.top, self.frame.size.width, self.frame.size.height - self.edgeInsets.top - self.edgeInsets.bottom);
    return _scrollView;
}
