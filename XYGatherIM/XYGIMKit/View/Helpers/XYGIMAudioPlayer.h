...
@protocol XYGIMAudioPlayerDelegate <NSObject>

- (void)audioPlayerStateDidChanged:(XYGIMNVoiceMessageState)audioPlayerState forIndex:(NSUInteger)index;

@end

@interface XYGIMAudioPlayer : NSObject

@property (nonatomic, weak) id<XYGIMAudioPlayerDelegate> delegate;
 ...
 */
@property (nonatomic, assign) XYGIMNVoiceMessageState audioPlayerState;
