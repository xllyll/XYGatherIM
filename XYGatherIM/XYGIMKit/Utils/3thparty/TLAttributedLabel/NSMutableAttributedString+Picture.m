...
- (NSMutableArray *)setImageAlignment:(TLImageAlignment)imageAlignment
    ...
    for (TLAttributedImage *imageData in images) {
        ...
        NSDictionary *keyDic = [NSMutableAttributedString faceMap];
        
        NSString *value = keyDic[[NSString stringWithFormat:@"[/%@]",imageData.imageName]];
        if (value==nil) {
            value = imageData.imageName;
        }
        imageData.imageName = [NSString stringWithFormat:@"%@",value];
        
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"%@",value]];
//        value = imageData.imageName;
        // 判断图片是否存在
        if (!image) {
            continue;
        ...
        NSLog(@"%@",imageAttString);
        // imageAttString替换文字AttString
        
        NSString *imageStr = [NSString stringWithFormat:@"%@%@%@", TLPictureBeginFlag,imageName, TLPictureEndFlag];
...
+(NSDictionary*)faceMap{
    ...
    {
        emojiFilePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"_expression_cn.plist"];
    }else{
        emojiFilePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"_expression_cn.plist"];
    ...
    NSDictionary *emojiDictionary = [[NSDictionary alloc] initWithContentsOfFile:emojiFilePath];
