...
+ (XYNetconnectionType)getNetconnectionType {
    XYNetconnectionType netconnType;
    Reachability *reach = [Reachability reachabilityWithHostName:@"www.apple.com"];
    
    switch ([reach currentReachabilityStatus]) {
        case NotReachable:
            netconnType = XYNetconnectionTypeNone;
            break;
        case ReachableViaWiFi:
            netconnType = XYNetconnectionTypeWifi;
            break;
            // 手机自带网络
        case ReachableViaWWAN:
        {
            ...
            if ([currentStatus isEqualToString:@"CTRadioAccessTechnologyGPRS"]) {
                //                netconnType = @"GPRS";
                netconnType = XYNetconnectionTypeOther;
            }else if ([currentStatus isEqualToString:@"CTRadioAccessTechnologyEdge"]) {
                //                netconnType = @"2.75G EDGE";
                netconnType = XYNetconnectionType2G;
            }else if ([currentStatus isEqualToString:@"CTRadioAccessTechnologyWCDMA"]){
                //                netconnType = @"3G";
                netconnType = XYNetconnectionType3G;
                ...
                //                netconnType = @"4G";
                netconnType = XYNetconnectionType4G;
            }
        }
            break;
...
+ (void)saveImageToPhotoAlbum:(UIImage *)image {
    UIImageWriteToSavedPhotosAlbum(image, [XYUtils sharedUtils], @selector(image:didFinishSavingWithError:contextInfo:), nil);
}

+ (void)saveVideoToPhotoAlbum:(NSString *)videoPath {
    UISaveVideoAtPathToSavedPhotosAlbum(videoPath, [XYUtils sharedUtils], @selector(video:didFinishSavingWithError:contextInfo:), nil);
}

- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    if (error) {
        [XYUtils showMessageAlertWithTitle:nil message:@"保存照片失败"];
    }else {
        [XYUtils showActionSuccessHUD:@"已保存到系统相册"];
    }
}
...
- (void)video:(NSString *)videoPath didFinishSavingWithError:(NSError *)error
  contextInfo:(void *)contextInfo {
    if (error) {
        [XYUtils showMessageAlertWithTitle:nil message:@"保存视频失败"];
    }else {
        [XYUtils showActionSuccessHUD:@"已保存到系统相册"];
    }
}
