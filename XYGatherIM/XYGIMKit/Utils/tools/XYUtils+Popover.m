...
@implementation XYUtils (Popover)
...
+ (void)showTipView:(UIView<XYTipDelegate> *)view {
    [XYTipView showTipView:view];
}

+ (void)hideTipView:(UIView<XYTipDelegate> *)tipView {
    [XYTipView hideTipView:tipView];
}
...
+ (MBProgressHUD *)progressHUDInView:(UIView *)view {
    ...
    if (!hudView) {
        ...
        if (!hudView.superview) {
            [XYUtils addViewToPopOverWindow:hudView];
        }
...
- (void)hudWasHidden:(MBProgressHUD *)hud {
    UIView *hudView = [XYUtils defaultHUDView];
    
    if (hudView.subviews.count == 0) {
        if (hudView.window == [XYUtils popOverWindow]) {
            [XYUtils removeViewFromPopOverWindow:hudView];
        }else {
